<template>
  <section ref="sectionRef" class="relative bg-dark overflow-hidden no-snap">
    ...
    <div class="text-center pt-28 pb-14 md:pt-36 md:pb-20 px-6">
      <p
        class="gallery-script font-sans text-xs md:text-sm uppercase tracking-[0.4em] text-gold/60 mb-4"
        :class="{ 'font-khmer! text-sm! tracking-normal!': locale === 'kh' }"
      >
        {{ t("gallery.subtitle") }}
      </p>
      <h2
        class="gallery-title font-display text-5xl md:text-7xl text-warm-white font-semibold"
    ...
    <div ref="galleryViewport" class="relative h-screen w-full overflow-hidden">
      ...
      >
        <div class="absolute inset-0 overflow-hidden">
          <img
            :src="image.src"
            :alt="galleryItems[Number(index)]?.alt ?? image.alt"
        ...
        <!-- Gradient overlay -->
        <div
          class="absolute inset-0 bg-gradient-to-t from-black/60 via-black/10 to-black/30"
        />
        ...
        <div class="slide-caption absolute bottom-0 left-0 right-0 p-8 md:p-14">
          <div class="max-w-2xl mx-auto">
            <p
              class="font-sans text-[10px] uppercase tracking-[0.3em] text-gold/60 mb-3"
              :class="{
                'font-khmer! text-xs! tracking-normal!': locale === 'kh',
              }"
            >
              {{ galleryItems[Number(index)]?.date }}
            </p>
            <p
              class="font-display text-xl md:text-3xl text-white/90 leading-snug"
              :class="{ 'font-khmer! text-lg! md:text-2xl!': locale === 'kh' }"
            >
              {{ galleryItems[Number(index)]?.alt }}
            </p>
          </div>
        </div>
      </div>
      ...
      <!-- Progress dots -->
      <div
        class="absolute right-6 top-1/2 -translate-y-1/2 z-50 flex flex-col gap-2"
      >
        <div
